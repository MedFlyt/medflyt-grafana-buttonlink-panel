...
test('should display "No data" in case panel data is empty', async ({
  ...
  await expect(panelEditPage.panel.locator).toContainText('Click Me!');
});
...
//   await panelEditPage.setVisualization('Href');
...
//   await panelEditPage.datasource.set(ds.name);
//   await panelEditPage.setVisualization('Href');
//   await panelEditPage.collapseSection('Href');
//   await expect(page.getByTestId('simple-panel-circle')).toBeVisible();
//   const showSeriesSwitch = panelEditPage
//     .getByGrafanaSelector(selectors.components.PanelEditor.OptionsPane.fieldLabel('Href Show series counter'))
